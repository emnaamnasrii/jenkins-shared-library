...
def call() {
    ...
    if (pythonFiles.size() > 0) {
        ...
        if (fileExists("requirements.txt")) {
            ...
            if (req.contains("fastapi")) framework = "fastapi"
            else if (req.contains("django")) framework = "django"
            else if (req.contains("flask")) framework = "flask"
            else if (req.contains("streamlit")) framework = "streamlit"
        }

        echo "✅ Detected Python ${framework ?: ''}"
        return [language: language, framework: framework]  // on garde le framework pour Dockerfile
    }
    ...
    if (mavenFiles.size() > 0) {
        ...
        if (pom.contains("spring-boot")) framework = "springboot"
    ...
    if (gradleFiles.size() > 0) {
        ...
        return [language: language]  // pas de framework par défaut
    }
    ...
    if (findFiles(glob: '**/composer.json').size() > 0) {
        ...
        if (composer.contains("laravel")) framework = "laravel"
